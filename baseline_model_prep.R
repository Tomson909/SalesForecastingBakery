...
features_matrix_train <- model.matrix(Umsatz ~ Wochentag + Warengruppe + IsFerien + IsFeiertag + KielerWoche, data=train_data_combined)
...
features_matrix_validation <- model.matrix(Umsatz ~ Wochentag + Warengruppe + IsFerien + IsFeiertag + KielerWoche, data=validation_data_combined)

# Umwandlung in ein tibble
features_validation <- as_tibble(features_matrix_validation)

# Erstellen der Dummy-Variablen und Hinzufügen der numerischen Variablen für validation_data_combined
features_matrix_test <- model.matrix(~ Wochentag + Warengruppe + IsFerien + IsFeiertag + KielerWoche, data=test_data_combined)
...
write.csv(features_train, "features_train_basic.csv", row.names = FALSE)

# Exportieren von label_train als CSV
write.csv(label_train, "label_train_basic.csv", row.names = FALSE)

# Exportieren von label_validation als CSV
write.csv(label_validation, "label_validation_basic.csv", row.names = FALSE)

# Exportieren von features_validation als CSV
write.csv(features_validation, "features_validation_basic.csv", row.names = FALSE)

# Exportieren von features_test als CSV
write.csv(features_test, "features_test_basic.csv", row.names = FALSE)


#Exportieren von test_data_combined für die IDs
write.csv(test_data_combined, "test_data_IDs_basic.csv", row.names = FALSE)
